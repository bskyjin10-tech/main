...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 모바일 음성 낭독 (TTS) 엔진 최적화
  let activeUtterance = null;

  // iOS 및 모바일 오디오 하드웨어 세션 활성화 (무음 모드 및 하드웨어 절전 해제)
  ...
  // 화면 첫 터치 시 오디오 세션 예열
  document.addEventListener('touchstart', activateAudioSession, { once: true, passive: true });
  document.addEventListener('click', activateAudioSession, { once: true, passive: true });

  // 음성 낭독 (TTS)
  function speakQuote(quote) {
    if (!('speechSynthesis' in window)) {
      showToast('이 브라우저는 음성 읽기를 지원하지 않습니다. Chrome이나 Safari로 열어주세요.');
      return;
    }

    try {
      // 터치 이벤트 컨텍스트에서 즉시 오디오 언락
      activateAudioSession();

      // 이전 음성 정리 및 오디오 컨텍스트 재개
      ...
      // 모바일 가비지 컬렉터(GC)에 의한 음성 중단 버그 방지
      ...
      utterance.onstart = () => {
        showToast('🔊 명언을 낭독합니다. (소리가 안 나면 볼륨 및 무음 스위치를 확인해 주세요)');
      };
      ...
      utterance.onerror = (e) => {
        ...
        if (e.error !== 'interrupted' && e.error !== 'canceled') {
          showToast('음성 낭독 중 문제가 발생했습니다. 다시 눌러보세요.');
        }
      };

      // 사용자 터치 즉시 동기 실행 (모바일 브라우저 필수 정책)
      window.speechSynthesis.speak(utterance);

    } catch (err) {
      console.error('TTS error:', err);
      showToast('음성을 재생할 수 없습니다.');
    }
  }
